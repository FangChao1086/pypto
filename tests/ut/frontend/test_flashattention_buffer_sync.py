...
from pypto.language.manual.buffer_policy import BufferType, SyncType
...
def test_double_buffer_kernel(
    ...
    with pl.section_vector():
        ...
        policy = plm.BuffersPolicyDB(tile_type, BufferType.UB, SyncType.INNER_CORE_SYNC)
...
def test_triple_buffer_kv_reuse_pattern_kernel(
    ...
    with pl.section_cube():
        ...
        policy = plm.BuffersPolicy3buff(tile_type, BufferType.L1, SyncType.INNER_CORE_SYNC)
...
def test_four_buffer_kernel(
    ...
    with pl.section_cube():
        ...
        policy = plm.BuffersPolicy4buff(tile_type, BufferType.L0C, SyncType.INNER_CORE_SYNC)
...
def test_cross_core_both_sync_kernel(
    ...
    with pl.section_cube():
        tile_type = plm.TileType(shape=[64, 128], dtype=pl.FP16, target_memory=pl.MemorySpace.Mat)
        policy = plm.BuffersPolicyDB(tile_type, BufferType.L1, SyncType.CROSS_CORE_SYNC_BOTH)
...
def test_fifo_pipeline_kernel(
    ...
    with pl.section_vector():
        ...
        policy = plm.BuffersPolicy4buff(tile_type, BufferType.UB, SyncType.INNER_CORE_SYNC)
